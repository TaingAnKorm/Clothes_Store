...
export class ProductListPage {
  ...
  private attachControlsEventListeners(): void {
    ...
    const searchInput = controls.querySelector("#search-input") as HTMLInputElement;
    const categorySelect = controls.querySelector("#category-select") as HTMLSelectElement;
    const minPriceInput = controls.querySelector("#min-price") as HTMLInputElement;
    const maxPriceInput = controls.querySelector("#max-price") as HTMLInputElement;
    const sortSelect = controls.querySelector("#sort-select") as HTMLSelectElement;
    ...
    const clearFilters = controls.querySelector("#clear-filters") as HTMLButtonElement;
  ...
  private renderProducts(): void {
    this.productsContainer.className = this.viewMode === "grid"
        ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6"
        : "space-y-4";

    if (this.loading) {
      this.productsContainer.innerHTML = Array(12).fill(0).map(() => new ProductCardSkeleton().create().outerHTML).join("");
      return;
    }
    if (this.error) {
        this.showError();
    ...
    this.productsContainer.innerHTML = this.filteredProducts.map(p => new ProductCard(p).create().outerHTML).join("");
    if (window.lucide) {
  ...
  private updateHeader(): void {
    ...
    const capitalized = category ? category.charAt(0).toUpperCase() + category.slice(1) : "All Products";
  ...
  private populateCategoryDropdown(): void {
      const select = this.page.querySelector("#category-select") as HTMLSelectElement;
      if (!select) return;
  ...
  private setInitialValuesFromUrl(): void {
      const params = getUrlParams();
      (this.page.querySelector("#search-input") as HTMLInputElement).value = params.get("search") || "";
      (this.page.querySelector("#min-price") as HTMLInputElement).value = params.get("min") || "";
      (this.page.querySelector("#max-price") as HTMLInputElement).value = params.get("max") || "";
  }
  ...
  private updateActiveFilters(): void {
      ...
      const params = getUrlParams();
      const hasFilters = params.has("search") || params.has("category") || params.has("min") || params.has("max");
